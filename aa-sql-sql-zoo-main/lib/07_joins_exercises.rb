...
def travoltas_busiest_years
  ...
  SELECT yr, count(*)
  FROM movies
  JOIN castings ON movies.id IN (
  -- id's of movies with John Travolta in it  
    SELECT movie_id
    FROM castings
    WHERE actor_id = (
      -- John Travolta's actor_id
      SELECT id FROM actors WHERE name = 'John Travolta'
    )
  )
  GROUP BY yr
